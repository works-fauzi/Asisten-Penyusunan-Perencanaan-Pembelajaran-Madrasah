...
**Asisten Penyusunan Modul Ajar Madrasah** adalah aplikasi berbasis Web AI Generator yang dirancang khusus untuk membantu para pendidik, guru madrasah (MI, MTs, MA/MAK), serta penyusun kurikulum Kementerian Agama dalam menyusun **Modul Ajar** dan **Lembar Kerja Peserta Didik (LKPD)** secara cepat, terstruktur, dan elegan.
...
- 📄 **Ekspor MS Word (.docx) Standar MSO**: Menggunakan arsitektur penataan MSO HTML Standard yang menjamin hasil unduhan presisi pada ukuran A4, margin 1 inci, dan font Times New Roman tanpa cacat visual.
...
- 📊 **Visual Diagram Tabel CSS Murni**: Seluruh diagram alir, peta konsep, dan skema materi dikonversi menjadi tabel HTML border CSS presisi tanpa mengandalkan gambar eksternal yang rentan pecah.
...
- **Ekapsulasi Penuh Kode CSS**: Mengisolasikan seluruh aturan CSS di dalam tag `<head><style type="text/css">...</style></head>` serta mengeliminasi teks kode CSS yang berisiko bocor pada dokumen Microsoft Word saat diekspor.
...
1. **Proses Build Lokal**:
   - Buka terminal pada repositori komputer Anda dan jalankan skrip *build*:
     ...
     npm run build
     ```
   - Perintah ini akan menghasilkan folder produksi `dist/`.
