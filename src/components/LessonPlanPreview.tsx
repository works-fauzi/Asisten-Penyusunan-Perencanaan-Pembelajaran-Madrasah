...
export default function LessonPlanPreview({ plan, onUpdatePlan, isGenerating }: LessonPlanPreviewProps) {
  ...
  if (isGenerating) {
    return (
      <div id="preview-loading" className="bg-white border border-slate-100 rounded-xl shadow-sm p-12 text-center h-[650px] flex flex-col items-center justify-center space-y-6">
  ...
  if (!plan) {
    return (
      <div id="preview-empty" className="bg-white border border-slate-100 rounded-xl shadow-sm p-12 text-center h-[650px] flex flex-col items-center justify-center space-y-4">
        ...
        <div className="space-y-1.5 max-w-sm">
          <h3 className="font-semibold text-slate-700">Belum Ada Perencanaan yang Terpilih</h3>
          <p className="text-xs text-slate-500 leading-relaxed">
  ...
  return (
    <div id="preview-active" className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-[650px] overflow-hidden text-left">
      {/* Control Toolbar */}
      <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 shrink-0">
          ...
          {isEditing ? (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg transition-colors cursor-pointer"
              >
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#fdfdfd]">
        {saveSuccess && (
          <div className="mb-4 p-2.5 bg-emerald-50 border border-emerald-200 rounded-lg text-xs font-medium text-emerald-800 text-center animate-fade-in">
        ...
        {isEditing ? (
          <div className="h-full flex flex-col space-y-2">
            <div className="flex justify-between items-center text-[11px] text-slate-500 bg-slate-100 p-2 rounded">
            ...
            <textarea
              ...
              className="flex-1 w-full p-4 text-xs font-mono border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-sky-500/20 resize-none h-[420px]"
              placeholder="Gunakan format Markdown untuk menulis..."
            />
          </div>
        ) : (
          <div id="print-sheet" className="bg-white border border-slate-200 rounded-xl p-8 shadow-sm max-w-4xl mx-auto prose prose-slate max-w-none text-left min-h-full">
            {/* Header Badge */}
            <div className="border-b-2 border-sky-600 pb-3 mb-6 flex justify-between items-center">
              <div>
                ...
                <p className="text-[9px] text-slate-500">Integrasi Deep Learning & Kurikulum Berbasis Cinta</p>
              </div>
              <span className="text-[10px] font-bold bg-sky-50 text-sky-700 px-2 py-1 rounded uppercase">
